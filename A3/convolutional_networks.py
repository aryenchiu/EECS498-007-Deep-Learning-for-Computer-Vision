...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    for i in range(N):
        for j in range(F):
            for h in range(0, 1 + (H + 2 * pad - HH), stride):
                for k in range(0, 1 + (W + 2 * pad - WW), stride):
                    out[i, j, h // stride, k // stride] = \
                    torch.sum(w[j] * x_pad[i, :, h:h+HH, k:k+WW]) + b[j]
        ...
        for j in range(F):
            for h in range(H_out):
                for k in range(W_out):
                    dw[j, :] += dout[i, j, h, k] * x_pad[i, :, h*stride:h*stride+HH, k*stride:k*stride+WW]
        ...
        for j in range(C):
            for h in range(0, H - pool_height + 1, stride):
                for k in range (0, W - pool_width + 1, stride):
                    out[i, j, h // stride, k // stride] = torch.max(x[i, j, h:h+pool_height, k:k+pool_width])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            for h in range(H_out):
                for k in range(W_out):
                    submatrix = x[i, j, h*stride:h*stride+pool_height, k*stride:k*stride+pool_width]
                    mask = submatrix == torch.max(submatrix)
                    dx[i, j, h*stride:h*stride+pool_height, k*stride:k*stride+pool_width] = dout[i, j, h, k] * mask
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    # divided by 2 because of pooling
  ...
  def loss(self, X, y=None):
    ...
    dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dh1, cache1)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    if weight_scale == "kaiming":
        ...
        # each pooling half the size of the height and width
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
        ...
        if self.batchnorm:
            ...
            if i in self.max_pools:
                x, caches[i+1] = Conv_BatchNorm_ReLU_Pool.forward(x, w, b, gamma, beta, conv_param, self.bn_params[i], pool_param)
            else:
                x, caches[i+1] = Conv_BatchNorm_ReLU.forward(x, w, b, gamma, beta, conv_param, self.bn_params[i])
        else:
            if i in self.max_pools:
                x, caches[i+1] = Conv_ReLU_Pool.forward(x, w, b, conv_param, pool_param)
            else:
                x, caches[i+1] = Conv_ReLU.forward(x, w, b, conv_param)

    w = self.params[f'W{self.num_layers}']
    b = self.params[f'b{self.num_layers}']
    scores, caches[self.num_layers] = Linear.forward(x, w, b)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dh, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dout, caches[self.num_layers])
    grads[f'W{self.num_layers}'] += 2 * self.reg * self.params[f'W{self.num_layers}']
    for i in range(self.num_layers - 1, 0, -1):
        if self.batchnorm:
            if i-1 in self.max_pools:
                dh, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU_Pool.backward(dh, caches[i])
            else:
                dh, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU.backward(dh, caches[i])
        else:    
            if i-1 in self.max_pools:
                dh, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU_Pool.backward(dh, caches[i])
            else:
                dh, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU.backward(dh, caches[i])
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      cache = mode, x_normalized, x_mean, std, gamma
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    mode, x_normalized, x_mean, std, gamma = cache
    N, D = dout.shape
    ...
    if mode == 'train':
        ...
        d_norm = dout * gamma
        ddeno = torch.sum(d_norm * x_mean, dim=0)
        dsqrt = ddeno * (-1 / std ** 2)
        dmean2 = dsqrt * (0.5 / std)
        dsquare = dmean2 / N
        dminus_down = dsquare * 2 * x_mean
        dminus_up = d_norm / std
        dmean1 = -1 * torch.sum(dminus_up + dminus_down, dim=0)
        dx1 = dmean1 / N
        dx2 = dminus_up + dminus_down
        dx = dx1 + dx2
    elif mode == 'test':
        dbeta = torch.sum(dout, dim=0)
        dgamma = torch.sum(dout * x_normalized, dim=0)
