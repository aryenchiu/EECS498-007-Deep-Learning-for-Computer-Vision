...
def svm_loss_vectorized(W, X, y, reg):
  ...
  scores = X.mm(W)  # (N, C)
  yi_scores = scores[torch.arange(N), y].view(-1, 1) # (N, 1) for broadcasting
  margins = torch.max(torch.zeros_like(scores), scores - yi_scores + 1)
  ...
  margins[margins > 0] = 1   # binarize the margin matrix to count dyi
  count = margins.sum(dim=1)
  margins[torch.arange(N), y] = -count
  # s = XW
  # dL/dW = X^T x dL/ds , (D, C) = (D, N) x (N, C)
  dW = torch.mm(X.t(), margins)
  dW /= N
  dW += reg * 2 * W
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
      ...
      scores -= torch.max(scores)    # prevent numeric instability
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores -= torch.max(scores, dim=1, keepdim=True).values  # (N, C) - (N, 1)
