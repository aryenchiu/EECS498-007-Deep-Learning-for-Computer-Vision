...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    linear = x.mm(Wx) + prev_h.mm(Wh) + b
    next_h = torch.tanh(linear)
    cache = x, prev_h, Wx, Wh, linear
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    x, prev_h, Wx, Wh, linear = cache
    dnexth_tanh = dnext_h * (1 - torch.tanh(linear) ** 2)
    dx = dnexth_tanh.mm(Wx.t())
    dprev_h = dnexth_tanh.mm(Wh.t())
    dWx = x.t().mm(dnexth_tanh)
    dWh = prev_h.t().mm(dnexth_tanh)
    db = dnexth_tanh.sum(dim=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    # must specify device and dtype, or would not pass the threshold 
    # of relative error between autograd
    dh_i = torch.zeros((N, H), device="cuda:0", dtype=torch.float64)

    for t in reversed(range(T)):
        dh_i += dh[:, t, :]
        dx[t], dh_i, dWx[t], dWh[t], db[t] = rnn_step_backward(dh_i, cache[t])
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        self.feature_projection = nn.Linear(input_dim, hidden_dim, device=device, dtype=dtype)
        ...
        self.output_projection = nn.Linear(hidden_dim, vocab_size, device=device, dtype=dtype)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type in ['rnn', 'lstm']:
            ...
            h = self.model.forward(word_vectors, h0)
        else:
            # [N, input, 4, 4] -> [N, 4, 4, input] -> [N, 4, 4, hidden] -> [N, hidden, 4, 4]
            A = self.feature_projection(image_features.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
            h = self.model.forward(word_vectors, A)
    ...
    def sample(self, images, max_length=15):
        ...
        image_features = self.feature_extractor.extract_mobilenet_feature(images)
        if self.cell_type == 'rnn':
            h = self.feature_projection(image_features)
        elif self.cell_type == 'lstm':
            h = self.feature_projection(image_features)
        ...
        for i in range(max_length):
            ...
            if self.cell_type == 'rnn':
                h = self.model.step_forward(x, h)
            elif self.cell_type == 'lstm':
                h, c = self.model.step_forward(x, h, c)
            elif self.cell_type == 'attention':
                attn, attn_weights_all[:, i, :, :] = dot_product_attention(h, A)
                h, c = self.model.step_forward(x, h, c, attn)
            scores = self.output_projection(h)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        h[t], c = lstm_step_forward(x[:, t, :], h[t-1], c, Wx, Wh, b)
...
def dot_product_attention(prev_h, A):
    ...
    # attn -> context vector
    attn = flatten_A.bmm(attn_weights.reshape(N, D_a * D_a, 1)).reshape(N, H)
...
def attention_forward(x, A, Wx, Wh, Wattn, b):
    ...
    c = c0
    for t in range(T):
        attn, _ = dot_product_attention(h[t-1], A)
        h[t], c = lstm_step_forward(x[:, t, :], h[t-1], c, Wx, Wh, b, attn, Wattn)
    h = torch.stack(h).permute(1, 0, 2)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
