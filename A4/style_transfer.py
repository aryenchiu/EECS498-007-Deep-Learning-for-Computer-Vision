...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    # Replace "pass" statement with your code
    loss = 0.0
    for l, style_target, style_weight in zip(style_layers, style_targets, style_weights):
        gram = gram_matrix(feats[l])
        loss += style_weight * (gram - style_target).pow(2).sum()
    return loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def tv_loss(img, tv_weight):
    ...
    # Replace "pass" statement with your code
    _, _, H, W = img.shape
    vertical_var = (img[:, :, 1:H, :W] - img[:, :, :H-1, :W]).pow(2).sum()
    horizontal_var = (img[:, :, :H, 1:W] - img[:, :, :H, :W-1]).pow(2).sum()
    loss = tv_weight * (vertical_var + horizontal_var)
    return loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################