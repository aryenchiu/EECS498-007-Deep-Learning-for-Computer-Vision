...
def GenerateProposal(anchors, offsets, method='YOLO'):
  ...
  wnh = anchors[..., 2:] - anchors[..., :2]
  centers = anchors[..., :2] + wnh / 2
  if method == "YOLO":
      c_p = centers + offsets[..., :2]
      wh_p = wnh * torch.exp(offsets[..., 2:])
  elif method == "FasterRCNN":
      c_p = centers + offsets[..., :2] * wnh
      wh_p = wnh * torch.exp(offsets[..., 2:])

  proposals[..., :2] = c_p - wh_p / 2
  proposals[..., 2:] = c_p + wh_p / 2
...
def IoU(proposals, bboxes):
  ...
  proposals = proposals.repeat(1, 1, 1, 1, N).reshape(B, A, H, W, N, 4).reshape(B, -1, N, 4)
  bboxes = bboxes.reshape(B, -1, N, 5)
...
class SingleStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    # Replace "pass" statement with your code
    B = images.shape[0]
    self.anchor_list = self.anchor_list.to('cuda:0')
    ...
    _, _ = ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, neg_thresh=0.2, method='YOLO')
    conf_scores, offsets, class_scores = self.pred_network.forward(features, activated_anc_ind, negative_anc_ind)
    
    conf_loss = ConfScoreRegression(conf_scores, GT_conf_scores)
    reg_loss = BboxRegression(offsets, GT_offsets)
    anc_per_image = torch.prod(torch.tensor(anchors.shape[1:-1]))
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    # # [B, A, H, W, 4] -> [B, H, W, A, 4] -> [B, H*W*A, 4]
    proposals = proposals.permute(0, 2, 3, 1, 4).reshape(B, H*W*A, 4)
    # # [B, C, H, W] -> [B, H*W]
    classes = class_scores.argmax(1).reshape(B, H*W)
    ...
    for i in range(B):
        # thresholding by confidence score threshold, 'thresh'
        indices = conf_scores[i] > thresh
        conf_scores_i = conf_scores[i, indices]
        proposals_i = proposals[i, indices]
        classes_i = classes.unsqueeze(2).repeat(1, 1, A).reshape(B, H*W*A)[i, indices]

        # thresholding by NMS
        nms_indices = torchvision.ops.nms(proposals_i, conf_scores_i, nms_thresh)
        proposals_i = proposals_i[nms_indices]
        conf_scores_i = conf_scores_i[nms_indices].unsqueeze(1)
        classes_i = classes_i[nms_indices].unsqueeze(1)
...
def nms(boxes, scores, iou_threshold=0.5, topk=None):
  ...
  while len(sorted_indices) > 0:
      ...
      hsb = remaining_boxes[0]
      remaining_boxes = remaining_boxes[1:]
      hsb_idx = sorted_indices[0].item()
      sorted_indices = sorted_indices[1:]
      keep.append(hsb_idx)
      
      # IoU calculation between highest-scoring box and every remaining boxes
      area_hsb = (hsb[2] - hsb[0]) * (hsb[3] - hsb[1])
      w_i = torch.min(hsb[2], remaining_boxes[:, 2]) - torch.max(hsb[0], remaining_boxes[:, 0])
      w_i[w_i < 0] = 0
      h_i = torch.min(hsb[3], remaining_boxes[:, 3]) - torch.max(hsb[1], remaining_boxes[:, 1])
      h_i[h_i < 0] = 0
      area_i = w_i * h_i
      area_boxes = (remaining_boxes[:, 2] - remaining_boxes[:, 0]) * (remaining_boxes[:, 3] - remaining_boxes[:, 1])
      area_u = area_hsb + area_boxes - area_i
      iou = area_i / area_u

      # eliminate boxes that IoU above the threshold
