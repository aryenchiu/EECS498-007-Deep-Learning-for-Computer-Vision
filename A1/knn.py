...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
      dists[i] = ((x_train_flatten[i] - x_test_flatten) ** 2).sum(dim=1)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_square = (x_train_flatten ** 2).sum(dim=1).view(-1, 1).repeat(1, num_test)
  x_test_square = (x_test_flatten ** 2).sum(dim=1).repeat(num_train, 1)
  dists = x_train_square + x_test_square - 2 * x_train_flatten.mm(x_test_flatten.t())
...
def predict_labels(dists, y_train, k=1):
  ...
  indices = torch.topk(dists, k, dim=0, largest=False).indices
  y_pred = torch.mode(y_train[indices], dim=0).values
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
      ...
      for i in range(num_folds):
          ...
          acc = clf.check_accuracy(x_test, y_test, k)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for key, value in k_to_accuracies.items():
      k_mean_acc[key] = statistics.mean(value)
      
  best_k = max(k_mean_acc, key=k_mean_acc.get)
